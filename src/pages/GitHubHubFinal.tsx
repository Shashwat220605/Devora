...
export default function GitHubHubFinal() {
  ...
  const filtered = useMemo(() => {
    ...
    return q
      ? repos.filter((repo) =>
          `${repo.name} ${repo.full_name} ${repo.language || ""}`
            .toLowerCase()
            .includes(q),
        )
      : repos;
  ...
  const importRepo = async (repo: GitHubRepo) => {
    ...
    try {
      setBusy(true);
      setError("");
      const response = await api.post<{ filesImported: number }>(
        "/github/import",
        {
          projectId,
      ...
      setMessage(
        `Imported ${response.data.filesImported} files from ${repo.full_name}.`,
      );
    } catch (err: any) {
      setError(
        err.response?.data?.message ||
        ...
        <div className="mt-2 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <p className="mt-1 max-w-2xl text-sm text-zinc-500">
              Securely connect GitHub, browse repositories, import code, and
              manage repository workflows from Devora.
            </p>
          </div>
          ...
          <div className="mx-auto max-w-2xl rounded-2xl border border-white/10 bg-[#0f0f12] p-6">
            <div className="flex items-start gap-4">
              ...
              <div>
                ...
                <p className="mt-1 text-sm text-zinc-500">
                  Authorize Devora through GitHub. No personal access token
                  needs to be copied into Devora.
                </p>
              </div>
            </div>
            ...
            <div className="mt-6 rounded-xl border border-white/10 bg-black/20 p-4 text-sm text-zinc-400">
              ...
              <p className="mt-2 leading-6">
                GitHub handles the sign-in and permission screen. Devora keeps
                the resulting credentials encrypted on the backend and never
                needs your GitHub password.
              </p>
            </div>
          ...
          <>
            <div className="rounded-2xl border border-white/10 bg-[#0f0f12] p-5">
              <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                <div>
                  ...
                  <p className="mt-1 text-sm text-zinc-500">
                    Choose the Devora project that will receive imported files.
                  </p>
                </div>
                ...
                <div className="flex flex-col gap-2 sm:flex-row">
                  ...
                  <button
                    ...
                    disabled={loading}
                    className="flex items-center justify-center gap-2 rounded-xl border border-white/10 px-3 py-2.5 text-sm text-zinc-300 hover:bg-white/[0.04]"
                  >
                    {loading ? (
                  ...
                  <button
                    ...
                    disabled={busy}
                    className="flex items-center justify-center gap-2 rounded-xl border border-red-500/20 px-3 py-2.5 text-sm text-red-400 hover:bg-red-500/10"
                  >
                    <LogOut size={15} />
                    Disconnect
                  </button>
            ...
            <div className="mt-5 grid gap-4 lg:grid-cols-2">
              {filtered.map((repo) => (
                <div
                  key={repo.id}
                  className="rounded-2xl border border-white/10 bg-[#0f0f12] p-5 hover:border-white/20"
                >
                  ...
                  <p className="mt-4 min-h-10 text-sm text-zinc-500">
                  ...
                  <div className="mt-5 flex gap-2">
                    <button
                      ...
                      className="flex flex-1 items-center justify-center gap-2 rounded-xl bg-white px-3 py-2.5 text-sm font-medium text-black hover:bg-zinc-200 disabled:opacity-40"
                    >
                      <Upload size={15} />
                      Import
                    </button>
                    ...
                    <button
                      ...
                      disabled={!projectId}
                      className="flex-1 rounded-xl border border-white/10 px-3 py-2.5 text-sm text-zinc-300 hover:bg-white/[0.04] disabled:opacity-40"
                    >
                      Open workspace
                    </button>
                  </div>
            ...
            {!loading && filtered.length === 0 && (
              <div className="mt-5 rounded-2xl border border-dashed border-white/10 bg-[#0f0f12] p-10 text-center text-sm text-zinc-500">
