...
interface GitHubStatus { connected: boolean; username: string | null; oauthUrl: string | null; }
...
export default function GitHubHubFinal() {
  ...
  const [status, setStatus] = useState<GitHubStatus>({ connected: false, username: null, oauthUrl: null });
  ...
  const load = async () => {
    try {
      ...
      if (oauthState === "connected") setMessage("GitHub connected successfully.");
      else if (oauthState === "error") setError(oauthMessage || "GitHub authorization failed.");
      if (oauthState) window.history.replaceState({}, "", "/github");
      ...
      setError(err.response?.data?.message || "Unable to load GitHub.");
    }
  };
  ...
  const connect = async () => {
    try {
      ...
      if (!response.data.oauthUrl) throw new Error("GitHub OAuth is not configured");
      window.location.assign(response.data.oauthUrl);
    } catch (err: any) {
      setError(err.response?.data?.message || err.message || "Unable to start GitHub authorization.");
      setBusy(false);
    }
  };
  ...
  const disconnect = async () => {
    try {
      ...
      setStatus({ connected: false, username: null, oauthUrl: null }); setRepos([]); setMessage("GitHub disconnected.");
    } catch (err: any) {
  ...
  const importRepo = async (repo: GitHubRepo) => {
    ...
    try {
      setBusy(true); setError("");
      const response = await api.post<{ filesImported: number }>("/github/import", { projectId, repositoryUrl: repo.html_url });
    ...
    <Layout active="GitHub">
      <div className="border-b border-white/10 px-5 py-6 sm:px-8">
        ...
        <div className="mt-2 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div><h1 className="text-2xl font-semibold">GitHub</h1><p className="mt-1 max-w-2xl text-sm text-zinc-500">Securely connect GitHub, browse repositories, import code, and manage repository workflows from Devora.</p></div>
      ...
      <section className="p-5 sm:p-8">
        ...
        {!status.connected ? (
          <div className="mx-auto max-w-2xl rounded-2xl border border-white/10 bg-[#0f0f12] p-6">
            <div className="flex items-start gap-4"><div className="flex h-12 w-12 items-center justify-center rounded-xl bg-white text-black"><GitBranch size={24} /></div><div><h2 className="font-semibold">Connect GitHub</h2><p className="mt-1 text-sm text-zinc-500">Authorize Devora through GitHub. No personal access token needs to be copied into Devora.</p></div></div>
            <div className="mt-6 rounded-xl border border-white/10 bg-black/20 p-4 text-sm text-zinc-400"><p className="font-medium text-zinc-200">Secure OAuth connection</p><p className="mt-2 leading-6">GitHub handles the sign-in and permission screen. Devora keeps the resulting credentials encrypted on the backend and never needs your GitHub password.</p></div>
            <button onClick={() => void connect()} disabled={busy} className="mt-4 flex w-full items-center justify-center gap-2 rounded-xl bg-white px-4 py-3 text-sm font-medium text-black hover:bg-zinc-200 disabled:opacity-40">{busy ? <Loader2 size={16} className="animate-spin" /> : <GitBranch size={16} />}Continue with GitHub</button>
          </div>
        ) : (
          <>
            <div className="rounded-2xl border border-white/10 bg-[#0f0f12] p-5">
              <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between"><div><p className="text-sm font-medium">Repository manager</p><p className="mt-1 text-sm text-zinc-500">Choose the Devora project that will receive imported files.</p></div>
                <div className="flex flex-col gap-2 sm:flex-row"><select value={projectId} onChange={(e) => setProjectId(e.target.value)} className="rounded-xl border border-white/10 bg-[#101014] px-3 py-2.5 text-sm outline-none"><option value="">Select project</option>{projects.map((project) => <option key={project.id} value={project.id}>{project.name}</option>)}</select>
                  ...
                  <button onClick={() => void disconnect()} disabled={busy} className="flex items-center justify-center gap-2 rounded-xl border border-red-500/20 px-3 py-2.5 text-sm text-red-400 hover:bg-red-500/10"><LogOut size={15} />Disconnect</button></div>
              </div>
