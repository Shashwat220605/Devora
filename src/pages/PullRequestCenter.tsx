...
import { GitPullRequest, GitMerge, MessageSquare, RefreshCw, ExternalLink, Plus, X, Loader2, Sparkles, ShieldAlert, ShieldCheck } from "lucide-react";
...
type PRFile = { path: string; status: string; additions: number; deletions: number; changes: number; patch: string | null };
type PRDetail = PR & { mergeable: boolean | null; mergeableState: string | null; files: PRFile[]; comments: Array<{ id: number; author: string; body: string; createdAt: string }> };
type Review = { model: string; projectAware: boolean; summary: string; risk: "low" | "medium" | "high"; recommendation: string; findings: Array<{ severity: "blocker" | "warning" | "suggestion" | "info"; file: string; title: string; detail: string; suggestion: string }> };
...
export default function PullRequestCenter() {
  ...
  const runAIReview = async () => {
    ...
    try {
      ...
      const response = await api.post<Review>("/ai/pr-review", {
        title: selected.title,
        body: selected.body,
        files: selected.files,
      });
      setReview(response.data);
    } catch (err: any) { setError(err.response?.data?.message || "Unable to run AI review."); }
    ...
    <section className="p-5 sm:p-8">
      ...
      <div className="grid gap-5 xl:grid-cols-[420px_1fr]">
        ...
        <div className="rounded-2xl border border-white/10 bg-[#0f0f12] min-h-[650px] overflow-hidden">
          {!selected ? <div className="flex min-h-[650px] items-center justify-center text-sm text-zinc-600">Select a pull request to review its files and discussion.</div> : <div><div className="border-b border-white/10 p-6"><div className="flex items-start justify-between gap-4"><div><p className="text-xs text-zinc-600">#{selected.number}</p><h2 className="mt-1 text-xl font-semibold">{selected.title}</h2><p className="mt-2 text-sm text-zinc-500">{selected.head} → {selected.base} · {selected.author}</p></div><div className="flex gap-2"><button onClick={() => void runAIReview()} disabled={reviewBusy} className="inline-flex items-center gap-2 rounded-xl bg-white px-3 py-2 text-xs font-medium text-black disabled:opacity-40">{reviewBusy ? <Loader2 size={14} className="animate-spin"/> : <Sparkles size={14}/>} AI Review</button><a href={selected.url} target="_blank" rel="noreferrer" className="rounded-xl border border-white/10 p-2.5 text-zinc-400"><ExternalLink size={16}/></a></div></div>{selected.body && <p className="mt-5 whitespace-pre-wrap text-sm text-zinc-300">{selected.body}</p>}<div className="mt-5 flex flex-wrap gap-2"><button disabled={busy || selected.merged} onClick={() => void merge("merge")} className="inline-flex items-center gap-2 rounded-xl bg-white px-4 py-2.5 text-sm font-medium text-black disabled:opacity-40"><GitMerge size={15}/> Merge</button><button disabled={busy || selected.merged} onClick={() => void merge("squash")} className="rounded-xl border border-white/10 px-4 py-2.5 text-sm text-zinc-300 disabled:opacity-40">Squash</button><button disabled={busy || selected.merged} onClick={() => void merge("rebase")} className="rounded-xl border border-white/10 px-4 py-2.5 text-sm text-zinc-300 disabled:opacity-40">Rebase</button></div></div>
          ...
          {review && <div className="border-t border-white/10 bg-black/20 p-6"><div className="flex items-start justify-between gap-4"><div><p className="text-xs uppercase tracking-[0.18em] text-zinc-600">Devora AI Review</p><h3 className="mt-1 text-lg font-semibold">{review.summary}</h3><p className="mt-1 text-xs text-zinc-600">{review.model}{review.projectAware ? " · project-aware" : ""}</p></div><div className={`inline-flex items-center gap-2 rounded-full border px-3 py-1.5 text-xs ${review.risk === "high" ? "border-red-500/20 bg-red-500/10 text-red-300" : review.risk === "medium" ? "border-amber-500/20 bg-amber-500/10 text-amber-300" : "border-emerald-500/20 bg-emerald-500/10 text-emerald-300"}`}>{review.risk === "high" ? <ShieldAlert size={13}/> : <ShieldCheck size={13}/>} {review.risk} risk</div></div><div className="mt-4 space-y-3">{review.findings.length === 0 ? <div className="rounded-xl border border-emerald-500/20 bg-emerald-500/10 p-4 text-sm text-emerald-300">No material issues found in this review.</div> : review.findings.map((finding, index) => <div key={`${finding.file}-${index}`} className="rounded-xl border border-white/10 p-4"><div className="flex flex-wrap items-center gap-2"><span className="rounded-full border border-white/10 px-2 py-1 text-[10px] uppercase text-zinc-500">{finding.severity}</span><span className="font-mono text-xs text-zinc-500">{finding.file || "PR"}</span><span className="text-sm font-medium text-zinc-200">{finding.title}</span></div><p className="mt-2 text-xs leading-5 text-zinc-400">{finding.detail}</p>{finding.suggestion && <p className="mt-2 text-xs leading-5 text-zinc-500">Suggestion: {finding.suggestion}</p>}</div>)}</div><div className="mt-4 rounded-xl border border-white/10 p-4"><p className="text-xs uppercase tracking-[0.16em] text-zinc-600">Recommendation</p><p className="mt-1 text-sm text-zinc-300">{review.recommendation}</p></div></div>}
          </div>}
        </div>
      </div>
