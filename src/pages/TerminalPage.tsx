...
const COMMANDS = ["help", "clear", "pwd", "ls", "cd projects", "git status", "git branch", "git log", "npm run build", "npm test", "echo hello"];
...
function executeCommand(raw: string) {
  ...
  switch (command.toLowerCase()) {
    ...
    case "ls":
      return { clear: false, lines: [{ text: "projects/", tone: "muted" }, { text: "README.md", tone: "muted" }, { text: "package.json", tone: "muted" }] };
    case "cd":
      return { clear: false, lines: [{ text: rest ? `Directory changed to /devora/workspace/${rest.replace(/^\/+/, "")}` : "Usage: cd <directory>", tone: rest ? undefined : "error" }] };
    case "git":
      if (args[0] === "status") return { clear: false, lines: [{ text: "On branch main" }, { text: "Your working tree is ready for changes.", tone: "muted" }] };
      if (args[0] === "branch") return { clear: false, lines: [{ text: "* main" }, { text: "  develop", tone: "muted" }] };
      if (args[0] === "log") return { clear: false, lines: [{ text: "a4164ac  update production workspace" }, { text: "c59fc88  sync database schema", tone: "muted" }, { text: "80d6177  add dashboard tools", tone: "muted" }] };
      return { clear: false, lines: [{ text: "Supported git commands: status, branch, log", tone: "error" }] };
    case "npm":
      if (args[0] === "run" && args[1] === "build") return { clear: false, lines: [{ text: "> npm run build" }, { text: "Build command queued in safe preview mode.", tone: "muted" }, { text: "Real execution will be enabled with the secured terminal backend.", tone: "muted" }] };
      if (args[0] === "test") return { clear: false, lines: [{ text: "> npm test" }, { text: "Test command queued in safe preview mode.", tone: "muted" }, { text: "Real execution will be enabled with the secured terminal backend.", tone: "muted" }] };
      return { clear: false, lines: [{ text: "Supported npm commands: npm run build, npm test", tone: "error" }] };
    case "echo":
      return { clear: false, lines: [{ text: rest }] };
    default:
      return { clear: false, lines: [{ text: `Command not available in safe mode: ${value}`, tone: "error" }] };
  }
}
...
export default function TerminalPage() {
  ...
  const run = (input = command) => {
    ...
    if (result.clear) {
      ...
      setLines((current) => [...current, { text: `$ ${value}`, tone: "command" }, ...result.lines]);
    }
    setCommand("");
        ...
        <div className="border-b border-white/10 px-5 py-5 sm:px-8">
          <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              <button onClick={copyOutput} title="Copy output" className="rounded-lg border border-white/10 p-2 text-zinc-500 hover:bg-white/[0.04] hover:text-white"><Copy size={15} /></button>
          ...
          <div className="mx-auto flex h-[calc(100vh-220px)] max-w-6xl flex-col overflow-hidden rounded-2xl border border-white/10 bg-black shadow-2xl">
            ...
            <div className="border-t border-white/10 p-3">
              ...
              <div className="flex items-center gap-2 rounded-xl border border-white/10 bg-[#0b0b0d] px-3 py-2.5">
                ...
                <input value={command} onChange={(event) => { setCommand(event.target.value); setSuggestions(true); }} onFocus={() => setSuggestions(true)} onKeyDown={(event) => { if (event.key === "Enter") run(); if (event.key === "Escape") setSuggestions(false); }} placeholder="Type a command..." className="min-w-0 flex-1 bg-transparent font-mono text-sm text-white outline-none placeholder:text-zinc-700" autoComplete="off" />
              ...
              <p className="mt-2 text-[10px] text-zinc-700">Enter to run • Esc to close suggestions • Ctrl/Cmd + L clears the terminal</p>
            </div>
          </div>
        </section>
      </div>
