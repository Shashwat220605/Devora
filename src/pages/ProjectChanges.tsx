...
export default function ProjectChanges() {
  ...
  const loadDiff = async (
    ...
    try {
      ...
      setSelectedPath(
        response.data.changes[0]?.path ||
          null,
      );
    } catch (err: any) {
      console.error(
        "Failed to load GitHub diff:",
        err,
      );
    ...
    try {
      ...
      setMessage(
        `${response.data.message}: ` +
          `${response.data.created ?? 0} created, ` +
          `${response.data.updated ?? 0} updated, ` +
          `${response.data.removed ?? 0} removed.`,
      );

      await loadDiff(false);
    } catch (err: any) {
      console.error(
        "Failed to pull GitHub changes:",
        err,
      );
  ...
  const pushToGitHub = async () => {
    ...
    try {
      ...
      setMessage(
        `${response.data.message}: ` +
          `${response.data.filesPushed ?? 0} files pushed, ` +
          `${response.data.filesDeleted ?? 0} remote files deleted.`,
      );

      await loadDiff(false);
    } catch (err: any) {
      console.error(
        "Failed to push GitHub changes:",
        err,
      );
      ...
      <header className="flex h-14 shrink-0 items-center justify-between border-b border-white/10 bg-[#0f0f12] px-4 sm:px-6">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="hidden items-center gap-2 rounded-lg border border-white/10 px-3 py-2 text-xs text-zinc-300 transition hover:bg-white/[0.05] disabled:opacity-40 sm:flex"
          >
            {pulling ? (
              <Loader2
                size={14}
          ...
          <button
            ...
            className="flex items-center gap-2 rounded-lg bg-white px-3 py-2 text-xs font-medium text-black transition hover:bg-zinc-200 disabled:cursor-not-allowed disabled:opacity-40"
          >
            {pushing ? (
              <Loader2
                size={14}
        ...
        <div className="mx-auto max-w-7xl">
            ...
            <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
              <div>
                ...
                <p className="mt-2 max-w-2xl text-sm text-zinc-500">
                  Compare the Devora workspace
                  against GitHub before syncing.
                </p>
              </div>
          ...
          <div className="mt-5 grid min-h-[580px] gap-4 lg:grid-cols-[340px_minmax(0,1fr)]">
            ...
            <section className="min-w-0 overflow-hidden rounded-2xl border border-white/10 bg-[#0b0b0e]">
              ...
              {selectedChange ? (
                <div className="h-[540px] overflow-auto p-4">
          ...
          <p className="mt-4 text-xs text-zinc-700">
            Pull replaces local files with the
            selected GitHub branch. Push Mirror can
            also remove remote files, so it stays
            an explicit action.
          </p>
        </div>
      </main>
